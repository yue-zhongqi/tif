...
class LoraDictInjectedLinear(nn.Module):
    def __init__(self, lora_dict, linear_weight, linear_bias, in_features, out_features, r=4, dropout_p=0.1, scale=1.0):
        super().__init__()
        self.loras = nn.ModuleDict({})
        for cname in lora_dict.keys():
            self.loras[cname] = LoraInjectedLinear(
                in_features, out_features, linear_bias is not None, r, dropout_p, scale
            )
            up_weight, down_weight = lora_dict[cname][0], lora_dict[cname][1]
            self.loras[cname].linear.weight = linear_weight
            if linear_bias is not None:
                self.loras[cname].linear.bias = linear_bias
            self.loras[cname].lora_up.weight = nn.Parameter(
                up_weight.type(linear_weight.dtype)
            )
            self.loras[cname].lora_down.weight = nn.Parameter(
                down_weight.type(linear_weight.dtype)
            )
        self.name = cname

    def forward(self, input):
        return self.loras[self.name](input)
...
def load_all_loras(model, loras_dir, iteration, lora_name='unet',
                   target_replace_module=DEFAULT_TARGET_REPLACE,
                   r: Union[int, List[int]] = 4, scale=1.0,
                   filtering_fn=None):
    # add module name for filtering
    for name, submodule in model.named_modules():
        submodule._name = name

    loras_dict = {}
    lora_names = os.listdir(loras_dir)
    for name in lora_names:
        loras_dict[name] = torch.load(
            os.path.join(loras_dir, name, f'{lora_name}_{iteration}.pt')
        )
    loras_list = []
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if filtering_fn is not None:
            if not filtering_fn(_child_module._name):
                continue
        specific_lora_dict = {}
        for lname in lora_names:
            specific_lora_dict[lname] = [
                loras_dict[lname].pop(0), loras_dict[lname].pop(0)
            ]
        _tmp = LoraDictInjectedLinear(
            specific_lora_dict, _child_module.weight, _child_module.bias,
            _child_module.in_features, _child_module.out_features,
            r=r.pop(0) if isinstance(r, list) else r, scale=scale
        )
        _module._modules[name] = _tmp
        _module._modules[name].to(_child_module.weight.device)
        loras_list.append(_tmp)
    return loras_list
